...
describe('UserController', () => {
  ...
  describe('register', () => {
    test('register - devrait créer un nouvel utilisateur avec succès', async () => {
      const userData = {
        nom: 'Dupont',
        prenom: 'Jean',
      ...
      expect(mockFindByEmail).toHaveBeenCalledWith(userData.email);
      // Vérifier que le mock a été appelé avec n'importe quels arguments
      expect(mockHash).toHaveBeenCalled();
      expect(mockCreate).toHaveBeenCalledWith({
        ...userData,
        password: 'hashed_password'
      });
      expect(mockSign).toHaveBeenCalledWith({ userId: createdUser.id }, process.env.JWT_SECRET, { expiresIn: '24h' });
    ...
    it('devrait renvoyer une erreur 400 si l\'email est déjà utilisé', async () => {
      // Arrange
      const userData = {
        nom: 'Dupont',
        prenom: 'Jean',
      ...
      mockRequest.body = userData;
      mockFindByEmail.mockResolvedValue({ id: 2, ...userData });
      ...
      expect(mockResponse.json).toHaveBeenCalledWith({ message: 'Cet email est déjà utilisé' });
    });
  });
